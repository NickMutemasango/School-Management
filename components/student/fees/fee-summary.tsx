import type { LucideIcon } from "lucide-react";
import { Banknote, CalendarClock, PiggyBank, Receipt } from "lucide-react";

import { cn, formatCurrency, formatDate } from "@/lib/utils";
import type { FeeStatement } from "@/lib/data/student-fees";

interface SummaryTile {
  label: string;
  value: string;
  caption: string;
  icon: LucideIcon;
  tone: string;
  valueTone?: string;
}

export function FeeSummary({ statement }: { statement: FeeStatement }) {
  const settled = statement.outstanding <= 0;

  const tiles: SummaryTile[] = [
    {
      label: "Total Billed",
      value: formatCurrency(statement.totalBilled),
      caption: statement.term || "No term selected",
      icon: Receipt,
      tone: "bg-blue-50 text-blue-600 dark:bg-blue-950/50 dark:text-blue-400",
    },
    {
      label: "Paid",
      value: formatCurrency(statement.totalPaid),
      caption: "Received to date",
      icon: Banknote,
      tone: "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400",
    },
    {
      label: "Outstanding",
      value: formatCurrency(statement.outstanding),
      caption: settled ? "Nothing outstanding" : "Awaiting payment",
      icon: PiggyBank,
      tone: "bg-rose-50 text-rose-600 dark:bg-rose-950/50 dark:text-rose-400",
      valueTone: settled ? "text-emerald-600" : "text-rose-600",
    },
    {
      label: "Payment Due",
      value: statement.dueOn ? formatDate(statement.dueOn) : "—",
      caption: statement.dueOn ? "Settle by this date" : "No due date set",
      icon: CalendarClock,
      tone: "bg-amber-50 text-amber-600 dark:bg-amber-950/50 dark:text-amber-400",
    },
  ];

  return (
    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
      {tiles.map((tile) => {
        const Icon = tile.icon;
        return (
          <div
            key={tile.label}
            className="bg-background rounded-2xl border border-slate-200 p-5 shadow-sm dark:border-slate-800"
          >
            <div className="flex items-start justify-between gap-3">
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                {tile.label}
              </p>
              <div
                className={cn(
                  "grid size-10 shrink-0 place-items-center rounded-xl",
                  tile.tone
                )}
              >
                <Icon className="size-5" />
              </div>
            </div>

            <p
              className={cn(
                "mt-4 truncate text-3xl font-bold tracking-tight",
                tile.valueTone
              )}
            >
              {tile.value}
            </p>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
              {tile.caption}
            </p>
          </div>
        );
      })}
    </div>
  );
}
